...
self.addEventListener('fetch', function (event) {
  ...
  //            above (CACHE_NAME)
  event.respondWith(caches.open(CACHE_NAME).then((cacheOne) => { 
  // B8. TODO - If the request is in the cache, return with the cached version.
  //            Otherwise fetch the resource, add it to the cache, and return
  //            network response.
    return cacheOne.match(event.request).then((cacheTwo) => {
       return fetch(event.request).then((initialFetch) =>{
         cache.put(event.request, initialFetch.clone());
         return initialFetch;
       }) || cacheTwo;
    });
  }));
